import { sortScenario, searchScenarios, handleKeydown} from './utility.js'; // Adjust the path as necessary
import { createPostModal} from './gazo.js'; // Adjust the path as necessary
import { createRecipeModal} from './recipe.js'; // Adjust the path as necessary
...
$('.hunt, #return-button').click(function() {
    if ($(this).is('#return-button')) {
        ...
        if (meaningfulFilters.length === 0) {
            let message = $('<strong style="color: #0D1017; text-shadow: -0.5px -0.5px 0 #fff, 0.5px -0.5px 0 #fff, -0.5px 0.5px 0 #fff, 0.5px 0.5px 0 #fff; position: absolute; z-index: 1000; top: 50%; left: 50%; transform: translate(-50%, -50%);">Failed hunt, predator did not know what they were hunting.</strong>').hide().fadeIn(500);
            $('.search-area').prepend(message);
        ...
        Object.entries(sortedArchive).forEach(([postId, post], index) => {
            ...
            if (matchesFilter) {
                ...
                $post.on('click', function () {
                    ...
                    window.location.pathname = '/kari/' + newPostId + '.html';
                });
            }
